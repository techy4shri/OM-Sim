import sys
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QLineEdit, QPushButton, QFileDialog, QMessageBox, QApplication
import subprocess
...
class MainWindow(QWidget):
    ...
    def runApplication(self):
        ...
        if not appPath:
            QMessageBox.critical(self, "Inout Error",
        ...
        try:
            ...
            if not (0 <= startTime < stopTime < 5):
                raise ValueError(
                    'Oopsie daisy, start time must be less than stop time, and both should be in the range of [0,5]')
        ...
        try:
            result = subprocess.run([  # check the subprocess of open-modelica
                ...
                f"-override=startTime={startTime},stopTime={stopTime}"],
                capture_output=True,
                text=True,
                check=True
            )
            QMessageBox.information(
                self, "SUCCESS", f"Successful execution!\n\nOutput: \n{result.stdout}")
            ...
            print(
                'Please provide all inputs: application path, start time, and stop time')
